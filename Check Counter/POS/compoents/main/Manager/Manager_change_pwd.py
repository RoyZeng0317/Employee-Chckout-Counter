...
MANAGER_SCHEMA_PATH = os.path.join(BASE_DIR, "data", "manager.sql")
...
def 取得連線():
    if not os.path.exists(DB_PATH):
        messagebox.showerror("錯誤", "找不到店長資料庫，請先執行 check_counter_sql.py 建立資料庫")
        return None
    conn = sqlite3.connect(DB_PATH)
    cur = conn.cursor()
    cur.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='manager'")
    if cur.fetchone() is None:
        messagebox.showerror("錯誤", "找不到店長資料表，請先執行 check_counter_sql.py 建立資料庫")
        conn.close()
        return None
    return conn
...
def 修改密碼():
    ...
    try:
        cur = conn.cursor()
        cur.execute(
            "SELECT users_name, users_password FROM manager WHERE users_id = ?",
            (emp_id,),
        )
        row = cur.fetchone()
        if row is None:
            status_label.config(text="找不到此店長編號", fg="red")
            return
        ...
        cur.execute(
            "UPDATE manager SET users_password = ? WHERE users_id = ?",
            (new_hash, emp_id),
        )
        conn.commit()
...
tk.Label(form_frame, text="店長編號:").grid(row=0, column=0, sticky="e", padx=4, pady=8)
